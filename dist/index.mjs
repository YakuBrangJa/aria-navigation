...
function ButtonAria() {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: "ButtonAria from package" });
}

// src/aria/aria-root.ts
var AriaRoot = class {
  constructor(containerNode) {
    __publicField(this, "containerNode");
    __publicField(this, "navigationDisabled");
...
var aria_root_default = AriaRoot;

// src/aria/utils.aria.ts
var utils = {
...
var utils_aria_default = utils;

// src/aria/aria-grid.ts
var Grid = class extends aria_root_default {
  constructor(containerNode, props) {
    super(containerNode);
    __publicField(this, "grid");
    __publicField(this, "rowSelector");
    __publicField(this, "colSelector");
    __publicField(this, "focusedRow");
    __publicField(this, "focusedCol");
    __publicField(this, "shouldWrapCols");
     ...
     */
    __publicField(this, "getNextCell", (currRow, currCol, directionX, directionY) => {
    ...
    __publicField(this, "getNextVisibleCell", (directionX, directionY) => {
      ...
      while (this.isHidden(nextCell?.row, nextCell?.col)) {
        if (!nextCell) return;
        nextCell = this.getNextCell(nextCell?.row, nextCell?.col, directionX, directionY);
        if (nextCell?.col === nextCell?.row && nextCell?.col === nextCell?.col) {
          return null;
        }
      }
      return nextCell;
     ...
     */
    __publicField(this, "findFocusedItem", (focusedTarget) => {
    ...
    __publicField(this, "focusClickedCell", (event) => {
      if (!event.target) return;
      var clickedGridCell = utils_aria_default.findClosest(event.target, "[tabindex]");
      for (var row = 0; row < this.grid.length; row++) {
        for (var col = 0; col < this.grid[row].length; col++) {
          if (this.grid[row][col] === clickedGridCell) {
            this.setFocusPointer(row, col);
            if (!utils_aria_default.matches(clickedGridCell, "button[aria-haspopup]")) {
    ...
    __publicField(this, "setInitialFocus", () => {
    ...
    __publicField(this, "setFocusPointer", (row, col) => {
      if (!this.isValidCell(row, col)) {
        return false;
      }
      ...
      this.navigationDisabled = utils_aria_default.matches(this.grid[row][col], "input");
    ...
    __publicField(this, "checkFocusChange", (event) => {
      ...
      if (rowCaret === 0 && key === "ArrowUp") {
        this.handlers?.onReachStart && this.handlers.onReachStart();
        this.setInitialCell("last", colCaret);
        return;
      }
      if (rowCaret === this.grid.length - 1 && key === "ArrowDown") {
        this.handlers?.onReachEnd && this.handlers?.onReachEnd();
    ...
    __publicField(this, "handleClickCell", (event) => {
    ...
    __publicField(this, "clearEvents", () => {
    ...
    __publicField(this, "registerEvents", () => {
      this.clearEvents();
  ...
  isHidden(row, col) {
    var cell = this.containerNode?.querySelectorAll('tr, [role="row"]' /* ROW */)[row].querySelectorAll('th, td, [role="gridcell"]' /* CELL */)[col];
    return utils_aria_default.hasClass(cell, "hidden");
  }
  focusCell(row, col) {
    console.log(row, col);
  ...
  setupFocusGrid() {
    ...
    Array.prototype.forEach.call(this.containerNode?.querySelectorAll(this.rowSelector), (row) => {
      const rowCells = [];
      Array.prototype.forEach.call(row?.querySelectorAll(this.colSelector), (cell) => {
        const focusableSelector = "[tabindex]";
        if (utils_aria_default.matches(cell, focusableSelector)) {
          rowCells.push(cell);
        } else {
...
var aria_grid_default = Grid;

// src/aria/index.ts
function grid(containerNode, props) {
  const grid2 = new aria_grid_default(containerNode, props);
  return {
    registerEvents: grid2.registerEvents,
    clearEvents: grid2.clearEvents,
  };
}

// src/hooks/useAriaGrid.ts
var import_react = __toESM(require_react(), 1);
function useAriaGrid({ rowSelector, colSelector, onReachEnd, onReachStart }, depArray, ref) {
  const parentNode = ref || (0, import_react.useRef)();
