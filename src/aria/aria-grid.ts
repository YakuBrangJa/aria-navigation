 ...
 */
  ...
  focusClickedCell = (event: MouseEvent) => {
    if(!event.target) return;

    var clickedGridCell = utils.findClosest(event.target as HTMLElement, "[tabindex]") as HTMLElement;
  ...
  checkFocusChange = (event: KeyboardEvent) => {
    ...
    this.findFocusedItem(event.target as HTMLElement);
